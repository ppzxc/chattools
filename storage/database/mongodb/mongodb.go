...
import (
	...
	impl2 "github.com/ppzxc/chattools/storage/database/mongodb/repository/impl"
...
func NewMongoDbInstance(closable context.Context, authMechanism string, authSource string, host string, port int, username string, database string, password string, connectTimeout time.Duration, queryTimeout time.Duration) (database.Service, error) {
	...
	m.crudSequence = impl2.NewSequenceRepository(m.mongoDataBase, m.queryTimeout)
	m.crudUser = impl2.NewUserRepository(m.mongoDataBase, m.queryTimeout)
	m.crudFile = impl2.NewFileRepository(m.mongoDataBase, m.queryTimeout)
	m.crudTopic = impl2.NewTopicRepository(m.mongoDataBase, m.queryTimeout)
	m.crudNotify = impl2.NewNotifyRepository(m.mongoDataBase, m.queryTimeout)
	m.crudSubs = impl2.NewSubscriptionRepository(m.mongoDataBase, m.queryTimeout)
	m.crudMessage = impl2.NewMessageRepository(m.mongoDataBase, m.queryTimeout)
